...
import * as accessController from '../controllers/accessCode.controller';
...
// Adaptation des types Express/Multer.
// Le runtime reste exactement celui fourni par Multer.
const uploadFiles = upload.array('files') as unknown as RequestHandler;
...
router.patch(
  '/api/chat/conversations/:id/theme',
  requireAuth,
  chatController.updateConversationTheme
);
...
router.post(
  ...
  requireAuth,
  premiumController.activatePremiumKey
);
...
router.get(
  '/api/admin/premium-keys',
  requireAuth,
  requireAdmin,
  adminController.listPremiumKeys
);
...
router.post(
  ...
  requireAuth,
  requireAdmin,
  adminController.createPremiumKeys
);
