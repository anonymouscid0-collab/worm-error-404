...
type Tab = "stats" | "users" | "keys" | "conversations" | "settings" | "apikeys" | "ai";
...
export default function AdminPage() {
  ...
  const [aiTestMessage, setAiTestMessage] = useState("Dis bonjour et présente-toi en une phrase.");
    ...
    try {
      ...
      if (t === "ai") {
        ...
        const found = (key: string) =>
          settingsRes.data.settings.find((s: any) => s.key === key)?.value || "";
        setAiPrompt(found("ai_system_prompt"));
        setAiApiUrl(found("ai_api_url"));
      }
    } catch (err: any) {
  ...
  async function saveAiConfig() {
    setAiSaving(true);
    try {
      await api.put("/api/admin/settings", { key: "ai_system_prompt", value: aiPrompt });
      await api.put("/api/admin/settings", { key: "ai_api_url", value: aiApiUrl });
      if (aiApiKey.trim()) {
        await api.post("/api/admin/api-keys", {
          provider: aiProvider || "custom",
          label: "Clé IA active",
          keyValue: aiApiKey.trim(),
        });
        setAiApiKey("");
      }
    } finally {
  ...
  async function testAi() {
    ...
    setAiTestResult("");
    try {
      const res = await api.post("/api/admin/ai-test", { message: aiTestMessage });
      setAiTestResult(res.data.response);
    } catch (err: any) {
      setAiTestResult(err?.response?.data?.error ?? "Erreur lors du test.");
    } finally {
      setAiTesting(false);
    }
  }
      ...
      {tab === "apikeys" && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted">
            Utilise l'onglet "IA" pour ajouter une nouvelle clé active.
          </p>
        </div>
      )}
        ...
        <div className="max-w-2xl space-y-6">
          <div>
            ...
            <textarea
              ...
              rows={6}
              placeholder="Tu es WORM ERROR // 404, un développeur Full Stack Senior..."
              className="w-full rounded-lg border border-line bg-card px-4 py-3 text-sm outline-none focus:border-brand"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="https://api.exemple.com/v1/chat/completions"
              className="w-full rounded-lg border border-line bg-card px-4 py-3 text-sm outline-none focus:border-brand"
            />
          </div>
          ...
          <button
            ...
            disabled={aiSaving}
            className="rounded-full bg-brand px-6 py-3 text-sm font-medium text-white hover:bg-brand-dark disabled:opacity-50"
          >
            {aiSaving ? "Enregistrement..." : "Enregistrer la configuration"}
          </button>

          <div className="border-t border-line pt-6">
            <p className="mb-2 text-sm font-semibold text-ink">Tester l'IA</p>
            <div className="flex gap-3">
              <input
                value={aiTestMessage}
                onChange={(e) => setAiTestMessage(e.target.value)}
                className="flex-1 rounded-lg border border-line bg-card px-4 py-2.5 text-sm outline-none focus:border-brand"
              />
              <button
                onClick={testAi}
                disabled={aiTesting}
                className="rounded-full border border-brand px-5 py-2.5 text-sm font-medium text-brand hover:bg-brand-light disabled:opacity-50"
              >
                {aiTesting ? "..." : "Tester"}
              </button>
            </div>
            {aiTestResult && (
              <div className="mt-4 rounded-lg border border-line bg-card p-4 text-sm text-body">
                {aiTestResult}
              </div>
            )}
          </div>
